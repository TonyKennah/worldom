diff --git a/src/utils/keymap.py b/src/utils/keymap.py
index 0000000..0000001 100644
--- a/src/utils/keymap.py
+++ b/src/utils/keymap.py
@@ -1,36 +1,556 @@
-"""Minimal keymap with string→Binding parsing and polling helpers."""
...
+================
+
+This module parses human‑readable input expressions into `Binding`s and
+exposes both polling (is_action_down) and event‑based (event_to_actions)
+APIs. It supports:
+
+  - Keyboard (with CTRL/SHIFT/ALT/**GUI** modifiers)
+  - Mouse buttons (LMB/RMB/MMB/**MOUSE4/5** aliases)
+  - Mouse wheel vertical (**up/down**) and horizontal (**left/right**)
+  - Numpad & common punctuation keys
+  - Event matching on KEYDOWN/KEYUP and MOUSEBUTTONDOWN/MOUSEBUTTONUP
+
+Grammar examples
+----------------
+  "W"                   -> press W
+  "CTRL+SHIFT+K"        -> press K with CTRL&SHIFT
+  "ALT+F4"              -> the classic quit chord
+  "GUI+ENTER"           -> Cmd+Enter (macOS) / Win+Enter (Windows)
+  "MOUSE1" / "LMB"      -> left mouse button
+  "MOUSE4" / "MOUSE5"   -> extra mouse side buttons
+  "WHEEL_UP"            -> wheel up (event based)
+  "WHEEL_LEFT"          -> horizontal wheel left (trackpads)
+
+Design notes
+------------
+* Wheel bindings are **event‑only** (cannot be polled).
+* Diagnostics for invalid bindings are collected as warnings.
+* Bindings are bucketed by kind to reduce per‑event scans.
+"""
+
+from __future__ import annotations
+
