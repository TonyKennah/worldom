...
Traceback (most recent call last):
  ...
  File "C:\prj\WorldDom\src\core\map.py", line 34, in <module>
    from noise_adapter import Noise4
ModuleNotFoundError: No module named 'noise_adapter'
PS C:\prj\WorldDom> python .\main.py
pygame 2.6.1 (SDL 2.28.4, Python 3.13.5)
Hello from the pygame community. https://www.pygame.org/contribute.html
Warming up rendering libraries...
Rendering libraries are warm.
libpng warning: sBIT: invalid
Selected theme: Vulcan (vulcan)
Traceback (most recent call last):
  File "C:\prj\WorldDom\src\core\game.py", line 196, in worker_wrapper
    target_func(progress_state)
    ~~~~~~~~~~~^^^^^^^^^^^^^^^^
  File "C:\prj\WorldDom\src\core\game.py", line 410, in globe_rendering_worker  
    for progress in globe_renderer.render_map_as_globe(self.map.data, map_seed):
                    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^^ 
  File "C:\prj\WorldDom\src\rendering\globe_renderer.py", line 252, in render_map_as_globe
    colors_hash=_hash_colors(settings.GLOBE_TERRAIN_COLORS),
                ~~~~~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "C:\prj\WorldDom\src\rendering\globe_renderer.py", line 83, in _hash_colors
    for r, g, b in colors:
        ^^^^^^^
ValueError: too many values to unpack (expected 3)
Traceback (most recent call last):
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "C:\prj\WorldDom\src\core\game.py", line 254, in _run_loading_loop   
